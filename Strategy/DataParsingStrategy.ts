...
export class JsonParsingStrategy implements ParsingStrategy {
  parse(rawData: string): Animal[] {
    try {
      ...
      // Mapeamos los datos crudos a la interfaz Animal para asegurar que los campos coincidan
      return dataArray.map((item: any) => ({
        nombre: item.nombre || "Desconocido",
        especie: item.especie || "Desconocida",
        peso_kg: Number(item.peso_kg) || 0,
        n_patas: Number(item.n_patas) || 0,
        tipo: item.tipo || "Indefinido",
        esperanza_vida: item.esperanza_vida || "Desconocida"
      }));
